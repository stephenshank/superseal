...
class SuperReadGraph:
    def __init__(
            self, mapped_reads=None, covarying_sites=None, node_link_data=None
            ):
    ...
    def obtain_superreads(self, minimum_weight=3):
        ...
        for covarying_boundaries, read_group in read_groups.items():
            ...
            for read in read_group:
                ...
                shifted_sites = covarying_sites_in_read - read.reference_start
                value_at_covarying_sites = ''.join(fasta[shifted_sites])
                if value_at_covarying_sites in superreads:
                    superreads[value_at_covarying_sites] += 1
                else:
                    superreads[value_at_covarying_sites] = 1

            def admission(pair): return pair[1] >= minimum_weight
    ...
    def reduce(self):
        ...
        for edge in self.superread_graph.edges:
            source, target = edge
            self.superread_graph.edges[source, target]. \
    ...
    def candidate_quasispecies(self):
        G = self.superread_graph
        number_of_covarying_sites = max([
            G.nodes[node]['cv_end']
            for node in G.nodes
            if 'cv_end' in G.nodes[node]
        ])
    ...
    def get_full_superreads(self, consensus):
        consensus_np = np.array(list(str(consensus.seq)))
        superread_info = self.superreads
        superread_records = []

        for i, info in enumerate(superread_info):
            current_sequence = np.array(
                len(consensus.seq) * ['-'], dtype='<U1'
            )
            start = self.covarying_sites[info['cv_start']]
            end = self.covarying_sites[info['cv_end']-1]
            current_sequence[start: end] = consensus_np[start: end]
            covarying_indices = self.covarying_sites[
                info['cv_start']: info['cv_end']
            ]
        ...
        return superread_records

    def get_cvs_superreads(self):
        superread_records = []
        G = self.superread_graph
        for node in G.nodes:
            if node != 'source' and node != 'target':
                info = G.nodes[node]
                current_sequence = np.array(
                    len(self.covarying_sites) * ['-'], dtype='<U1'
                )
                start = info['cv_start']
                end = info['cv_end']
    ...
    def get_candidate_quasispecies(self, consensus):
        consensus_np = np.array(list(str(consensus.seq)))
        candidate_quasispecies, describing_superreads = \
            self.candidate_quasispecies()
        superread_records = []
        for i in range(candidate_quasispecies.shape[0]):
            current_sequence = np.copy(consensus_np)
            current_sequence[self.covarying_sites] = \
                candidate_quasispecies[i, :]
            superread_records.append(SeqRecord(
                Seq(''.join(current_sequence)),
                id='candidate_%d' % i,
                description=''
            ))
        return superread_records, describing_superreads
